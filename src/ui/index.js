...
export class UiSystem {
  ...
  async init(ctx) {
    ...
    on('damage:dealt', (e) => {
      ...
      // Credit requires the player to have FIRED it. This used to infer "the
      // target is not the player, therefore I did it", which handed the player
      // every hitmarker, damage number, XP banner and killfeed row generated by
      // AI-on-AI fire — measured scoreUs:5 with zero shots fired.
  ...
  /**
   * True when a `damage:dealt` payload was FIRED by the local player.
   *
   * Deliberately strict: an absent `source` returns false. A round with no
   * attributed shooter must not be credited to anyone, which is exactly the
   * inference that produced phantom kills before identity was threaded through
   * physics.fireBullet().
   */
  _isPlayerSource(s) {
    if (!s) return false;
